...
- Public GitHub repo means the code (including the curated dataset and this spec) is visible to anyone. `backend/.env` (which holds the real API key) has always been gitignored and was never committed. `frontend/.env` (no secret — only a local API URL) was mistakenly committed before `frontend/.gitignore` existed; the implementation plan untracks it before the GitHub push as a precaution, even though it carries no secret.
